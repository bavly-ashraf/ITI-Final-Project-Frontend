...
const CartItemList = ({ product, DeleteItem }) => {
    ...
    return (
        <>
            ...
            <section className="flex justify-between shadow-lg shadow-bg-slate-200-500/50 my-4">
                {/* list item card */}
                <figure className="col-span-3">
                    <img
                        className="w-40 h-20 " src={product.productId.photo_url[0]} alt="picture"
                    />
                </figure>

                <div className="px-3 col-span-5">
                    <h2 className="custom-font-bold text-xl">{product.productId.name}</h2>
                    <div className="custom-font-bold text-xl">{product.productId.price * product.quantity} <span className='text-project-main-color'>{"EGP"}</span></div>
                </div>
